...
- **Saving and issuing are two separate writes, asked for on the Forecast Result page.** Both used to fire silently when the Client Quote page opened; issuing then moved to its own button on the preview, which meant asking for the quotation twice. The two acts are still distinct in the database, but the user reaches them from one page:
  ...
  Every save still funnels into one `persistForecastRecord()`, including the fallback button inside the preview's draft panel.

  The gap between saving and issuing is not ceremony — it is the window in which the record is still the analyst's own. A saved-but-un-issued request can be edited or deleted from Forecast History; the moment a price is issued, the lock trigger closes both doors. Which is why **Generate Client Quotation asks first**, naming the date, quantity, price and validity period about to be fixed: the confirm is the last point at which the record can still be changed, so it states what is being given up rather than asking an abstract "are you sure?". Re-quote paths (`regenerateQuotation()`, `requoteForDate()`) issue immediately for the same reason in reverse — the user has already asked for a new price rather than a new draft.
- **A menu link never issues anything.** The sidebar's Client Quote Preview link is plain navigation. Only the button says the quotation should exist, so only the button creates one; a forecast reached through the link finds the preview's own draft panel offering the missing step (**Save Forecast Record**, then **Generate Quotation**). The same panel serves a Pending row reopened from Forecast History and an issue attempt that failed.
- **The preview shows the price before it is issued.** The draft panel carries the same three-column shape as an issued quotation — quoted date, quantity, total — but labelled *Estimated Price* under a `Draft` badge, against *Quoted Price* under a reference and an expiry. Withholding the figure until issue would mean pressing the irreversible button to find out what it does.
...
| Stage | View | Edit | Mark Cancelled | Delete |
|---|:--:|:--:|:--:|:--:|
| Pending, un-issued | ✅ | ✅ | ✅ | ✅ |
| Pending, issued | ✅ | — | ✅ | ✅ |
| Confirmed / Cancelled | ✅ | — | — | — |
...
- **Cancel and Delete are different acts, not two names for one.** Cancelling records that the client said no — an outcome worth keeping. Deleting says the request should never have existed. That is why cancelling is available on rows that can also be deleted. The button is worded **Mark as cancelled by client** rather than "Cancel", which beside a Delete button read as a dialog's dismiss control instead of the business decision it records.
- **Both ask first, in the app's own dialog.** `openConfirm()` drives one shared modal for the two irreversible row actions. It names the record — company, date, and for an issued quotation its reference and price — and runs the action from inside itself, so a refusal from the database is reported in the box the user was deciding in rather than as a detached alert. It replaced `window.confirm()`, which cannot be styled, cannot mark up the record it is asking about, and looks like a browser security warning.
