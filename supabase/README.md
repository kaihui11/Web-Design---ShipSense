...
| File | Role |
|---|---|
| `schema.sql` | Creates all three tables, enables RLS, adds policies and the update/delete guard triggers. Safe to re-run. |
| `seed.sql` | Generated by `scripts/pkl_to_seed_sql.py`. Fills all three tables in one paste. Run after `schema.sql`. Safe to re-run. |
