...
One file, loaded by both halves of the site: `app.html` (app.js, exec-data.js) and `contact.html` (site.js). Leaving either blank breaks the app entirely — the Executive Dashboard's Forecast and Historical Data tabs, and Forecast History/quote persistence, all read and write Supabase directly with no other data source — see `app.js`/`exec-data.js`.
